...
function FindByLocation() {
  ...
  const [sort, setSort] = useState(sortOptions[2]); //default sort by date published
  ...
  // useEffect(() => {
  //   console.log(location.location,subject.subject, );
  //   if (location.location && !posts) {
  //     const fetchPostByLocation = async () => {
  //       try {
  //         const response = await fetch(`/allpost?address=${location.location}&subject=${subject.subject}`);
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        setLoading(true);
        try {
          const sortValue = sort.value === "Price: Low to High" ? "asc" : "";
          const response = await fetch(
            `/allpost?address=${location.location}&subject=${
              subject.subject
          ...
          const data = await response.json();
          console.log(data);
          //if sort.sortotp is deadline then sort to nearest date from current date
          // if(sort.sortotp === "deadline"){
          //   data.posts.sort((a,b)=>{
          //     //include posts which have deadline greater than current date or equal to current date
    ...
    <Container fluid>
      <div className="breadcrumbContainer">
        <Breadcrumb className="text-muted text-decoration-none">
          ...
          <Breadcrumb.Item
            className="text-muted text-decoration-none"
            onClick={() => {
              navigate(`/category/${subject.subject}`);
            }}
          >
            {subject.subject}
          </Breadcrumb.Item>
      ...
      <container>
        <div className="d-flex justify-content-between align-items-center">
          <div className="results">
            <h4 className="text-dark fw-bold my-0 d-inline ">
              {subject.subject}
              <span className="text-muted  fs-6"> in </span>
              {location.location}
            </h4>
        ...
        <Button
          ...
          onClick={() => {
            fetch(
              `/allpost?subject=${subject.subject}&address=${
                location.location
              }&per_page=${perPage + 8}&sortBy=${sort.sortotp}&sortOrder=${
